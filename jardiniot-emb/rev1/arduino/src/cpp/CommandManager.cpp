...
String CommandManager::executeCommand(String command)
{
    ...
    {
    case ADD:
        msg = this->addController(*jCommand);
        break;
    case CONFIG:
        msg = this->modifyController(*jCommand);
        break;
    case DELETE:
        msg = this->removeController(*jCommand);
        break;
    }
    delete jCommand;
    return msg;
...
String CommandManager::addController(JardinCommand& jCommand)
{
    ...
    if (Sensor == controlType)
    {
        SensorStrategy* sensor = this->factory->createSensor(jCommand.getIdController(), jCommand.getControllerType(), jCommand.getDelay(),
                                                             jCommand.getInputPin(), jCommand.getOutputPin());
        this->sensorList.push_back(sensor);
    }
    else if (Motor == controlType)
    {
        // Fait la création des fonctions pour les moteurs
        MotorStrategy* motor = this->factory->createMotor(jCommand.getIdController(), jCommand.getControllerType(), jCommand.getDelay(),
                                                          jCommand.getInputPin(), jCommand.getOutputPin());
