...
bool JardinCommand::validIdController( std::vector<std::string> &vecCommand ) {
  if ( vecCommand[0] == "id" ) {
    ...
    // Si le idController est 0 mais que la chaine de caractères n'était pas 0
    // alors erreur de conversion
...
bool JardinCommand::validConversionStrToInt( std::string &str, int i ) {
  bool isValid = true;
  // Si la conversion donne 0 et que la chaine de caractère n'égale pas 0 alors
