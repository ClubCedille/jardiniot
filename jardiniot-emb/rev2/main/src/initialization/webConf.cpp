...
webConf::webConf() : portNumber(0), address(), listeArguments(), newConnectionSSID(), newConnectionPW(), rawGetRequest(), mqttIp(), mqttCredentials()
{
}
...
bool webConf::configure(int portNum)
{
    ...
    if ((incomingConnection = accept(socketFileDesc, (struct sockaddr*)&address, (socklen_t*)&addrlen)) < 0)
    {
        return ret;
    }
