...
bool webConf::configure( int portNum ) {
  ...
  int opt = 1;
  int err = setsockopt( socketFileDesc, SOL_SOCKET, SO_REUSEADDR, (void *) &opt,
                        sizeof( int ) );
  if ( err < 0 ) {
    return ret;
  }
  ...
  const std::string staticPage =
      ...
      "type=\"password\" id=\"wifipasswd\" name=\"PW\"><br><br>\n</div>\n<div "
      "class=\"EntryField\">\n<label for=\"MQTTIP\">MQTT URL :</label>\n<input "
      "type=\"text\" id=\"wifi passwd\" name=\"MQTTIP\"><br><br>\n</div>\n<div "
...
std::string webConf::getSSID( ) {
  std::string ret =
      this->newConnectionSSID.substr( this->newConnectionSSID.find( '=' ) + 1 );
  return ret;
}
